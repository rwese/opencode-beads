...
export const bd_show: ToolDefinition = tool({
  description:
    'Show detailed information about one or more issues. Returns formatted output for each requested issue separated by "---" delimiters. Always returns an array response even when requesting a single issue ID.',
  args: {
    id: tool.schema
      .array(tool.schema.string())
      .describe(
        'Issue ID(s) to retrieve (REQUIRED - can be single ID like "opencode-beads-abc" or array like ["opencode-beads-abc", "opencode-beads-def"])'
      ),

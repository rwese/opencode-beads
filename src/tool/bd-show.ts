...
import { runBd, formatOutput, transformBdIssue, transformBdIssues, isSuccess, handleBdError } from "./utils"
...
export const bd_show: ToolDefinition = tool({
  ...
  args: {
    id: tool.schema.array(tool.schema.string()).describe("Issue ID(s) to retrieve (REQUIRED - can be single ID or array)"),
  ...
  execute: async (args) => {
    ...
    if (issues.length === 0) {
      throw new Error(`Issue(s) not found: ${ids.join(", ")}. The issue(s) may have been deleted or the IDs are incorrect.`)
    }
